"""
Comprehensive Migration Script to convert ALL base64 images to fal.ai CDN URLs.
This handles: art_studio_generations, character_gallery, books, pages.

Run with: cd /app/backend && python scripts/migrate_all_images_to_cdn.py

IMPORTANT: Ensure FAL_KEY is set in .env before running!
"""

import asyncio
import os
import sys
import logging
from motor.motor_asyncio import AsyncIOMotorClient
from dotenv import load_dotenv

# Add parent directory to path BEFORE loading dotenv
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Load environment variables FIRST
env_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '.env')
load_dotenv(env_path, override=True)

# Verify FAL_KEY is loaded
fal_key = os.environ.get('FAL_KEY', '')
if not fal_key:
    print("ERROR: FAL_KEY not found in environment!")
    print(f"Checked .env at: {env_path}")
    sys.exit(1)

# Show masked key for verification
if len(fal_key) > 20:
    masked = f"{fal_key[:10]}...{fal_key[-4:]}"
else:
    masked = f"{fal_key[:4]}...{fal_key[-2:]}" if len(fal_key) > 6 else "***"
print(f"Using FAL_KEY: {masked}")

# Now import fal_service (which will use the key we just loaded)
from fal_service import upload_image_to_fal, is_fal_configured

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

# Rate limiting to avoid overwhelming fal.ai
BATCH_SIZE = 5  # Reduced batch size to prevent rate limiting
DELAY_BETWEEN_BATCHES = 3  # seconds - increased delay
DELAY_BETWEEN_ITEMS = 0.5  # delay between individual uploads


async def migrate_collection(db, collection_name: str, fields: list) -> dict:
    """
    Migrate base64 images in a collection to CDN URLs.
    
    Args:
        db: MongoDB database connection
        collection_name: Name of the collection
        fields: List of field names that may contain base64 images
    
    Returns:
        dict with migration stats
    """
    coll = db[collection_name]
    stats = {"migrated": 0, "failed": 0, "skipped": 0, "auth_errors": 0}
    
    for field in fields:
        # Find documents with base64 images in this field
        query = {field: {"$regex": "^data:image"}}
        docs = await coll.find(query, {"_id": 1, field: 1}).to_list(1000)
        
        if not docs:
            logger.info(f"  {collection_name}.{field}: No base64 images found")
            continue
        
        logger.info(f"  {collection_name}.{field}: Found {len(docs)} base64 images to migrate")
        
        for i, doc in enumerate(docs):
            doc_id = doc["_id"]
            base64_image = doc.get(field, "")
            
            if not base64_image or not base64_image.startswith("data:image"):
                stats["skipped"] += 1
                continue
            
            try:
                # Upload to CDN
                cdn_url = await upload_image_to_fal(base64_image)
                
                # Update the document
                await coll.update_one(
                    {"_id": doc_id},
                    {"$set": {field: cdn_url, f"{field}_migrated_from_base64": True}}
                )
                
                stats["migrated"] += 1
                logger.info(f"    ✓ Migrated {i + 1}/{len(docs)}: {cdn_url[:50]}...")
                
                # Rate limiting
                await asyncio.sleep(DELAY_BETWEEN_ITEMS)
                
                if (i + 1) % BATCH_SIZE == 0:
                    logger.info(f"    Pausing for rate limit... ({i + 1}/{len(docs)} done)")
                    await asyncio.sleep(DELAY_BETWEEN_BATCHES)
                    
            except Exception as e:
                error_str = str(e).lower()
                
                # Check for auth errors
                if '401' in error_str or 'unauthorized' in error_str:
                    stats["auth_errors"] += 1
                    logger.error(f"    ✗ AUTH ERROR: FAL_KEY is invalid or expired!")
                    logger.error(f"      Please update FAL_KEY in .env and restart the migration.")
                    # Stop migration on auth errors - no point continuing
                    if stats["auth_errors"] >= 3:
                        logger.error("    Multiple auth errors detected. Stopping migration.")
                        return stats
                
                # Check for rate limiting
                elif '429' in error_str or 'rate limit' in error_str:
                    logger.warning(f"    Rate limited! Waiting 30 seconds...")
                    await asyncio.sleep(30)
                    # Retry this one
                    try:
                        cdn_url = await upload_image_to_fal(base64_image)
                        await coll.update_one(
                            {"_id": doc_id},
                            {"$set": {field: cdn_url, f"{field}_migrated_from_base64": True}}
                        )
                        stats["migrated"] += 1
                        logger.info(f"    ✓ Retry succeeded: {cdn_url[:50]}...")
                    except Exception as retry_e:
                        stats["failed"] += 1
                        logger.error(f"    ✗ Retry failed: {retry_e}")
                else:
                    stats["failed"] += 1
                    logger.error(f"    ✗ Failed doc {i + 1}: {e}")
    
    return stats


async def run_migration():
    """Run the full migration across all collections."""
    
    logger.info("=" * 60)
    logger.info("STARTING COMPREHENSIVE BASE64 TO CDN MIGRATION")
    logger.info("=" * 60)
    
    # Verify fal.ai is configured
    if not is_fal_configured():
        logger.error("fal.ai is not properly configured!")
        logger.error("Please ensure FAL_KEY is set in your .env file")
        return
    
    # Connect to MongoDB
    mongo_url = os.environ.get('MONGO_URL')
    db_name = os.environ.get('DB_NAME')
    
    if not mongo_url:
        logger.error("MONGO_URL not set!")
        return
    
    client = AsyncIOMotorClient(mongo_url)
    db = client[db_name]
    
    # Test connection
    try:
        await db.command("ping")
        logger.info(f"Connected to MongoDB: {db_name}")
    except Exception as e:
        logger.error(f"Failed to connect to MongoDB: {e}")
        return
    
    # Test fal.ai connection first with a small upload
    logger.info("Testing fal.ai connection...")
    try:
        # Create a tiny test image (1x1 pixel PNG)
        test_image = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="
        test_url = await upload_image_to_fal(test_image)
        logger.info(f"✓ fal.ai connection test successful: {test_url[:50]}...")
    except Exception as e:
        logger.error(f"✗ fal.ai connection test FAILED: {e}")
        logger.error("Please verify your FAL_KEY is valid and try again.")
        return
    
    # Collections and their image fields
    migrations = [
        ("art_studio_generations", ["image_url"]),
        ("character_gallery", ["image_url"]),
        ("books", ["cover_image", "back_cover_image"]),
        ("pages", ["image_url", "image_url_2", "image_url_3", "image_url_4"]),
        ("art_studio_gallery", ["image_url"]),
        ("pro_studio_characters", ["image_url", "source_image", "reference_image", "preview_url"]),
        ("pro_studio_scenes", ["image_url", "source_image"]),
    ]
    
    total_stats = {"migrated": 0, "failed": 0, "skipped": 0, "auth_errors": 0}
    
    for collection_name, fields in migrations:
        logger.info(f"\n--- Migrating {collection_name} ---")
        
        if collection_name not in await db.list_collection_names():
            logger.info(f"  Collection {collection_name} does not exist, skipping")
            continue
        
        stats = await migrate_collection(db, collection_name, fields)
        
        total_stats["migrated"] += stats["migrated"]
        total_stats["failed"] += stats["failed"]
        total_stats["skipped"] += stats["skipped"]
        total_stats["auth_errors"] += stats["auth_errors"]
        
        logger.info(f"  {collection_name}: migrated={stats['migrated']}, failed={stats['failed']}, skipped={stats['skipped']}")
        
        # Stop if we hit auth errors
        if stats["auth_errors"] >= 3:
            logger.error("\nStopping migration due to authentication errors.")
            break
    
    logger.info("\n" + "=" * 60)
    logger.info("MIGRATION COMPLETE")
    logger.info("=" * 60)
    logger.info(f"Total migrated: {total_stats['migrated']}")
    logger.info(f"Total failed: {total_stats['failed']}")
    logger.info(f"Total skipped: {total_stats['skipped']}")
    
    if total_stats["auth_errors"] > 0:
        logger.warning(f"Auth errors encountered: {total_stats['auth_errors']}")
        logger.warning("Please update your FAL_KEY and run the migration again for remaining items.")
    
    client.close()


if __name__ == "__main__":
    asyncio.run(run_migration())
