  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (isExploringRef.current && cameraRef.current) {
        ...
        // Ground floor is at Y ≈ 4.2, bridge/upper floor at Y ≈ 6.5
        // Player on ground floor has feet at Y ≈ 4.0-5.0, bridge at Y ≈ 5.5-7.0
        const playerIsGroundFloor = playerFootY < 5.5;
        ...
        // Compare by ID to ensure React detects the change
        const currentPortalId = nearPortal?.id || null;
        const foundPortalId = foundPortal?.id || null;
        
        if (foundPortalId !== currentPortalId) {
          if (foundPortal) {
            console.log('Near portal:', foundPortal.name, foundPortal.id);
          } else {
            console.log('Left portal area - hiding button');
          }
          setNearPortal(foundPortal || null);
        }
      }
      ...
      scene.traverse((child) => {
        ...
        if (child.userData?.isPortalRing && child.material) {
          ...
          // Ground floor Y ≈ 4.2, bridge Y ≈ 6.5
          const playerOnGround = (camera.position.y - PLAYER_HEIGHT) < 5.5;
                  ...
                  >
                    ...
                    >
                      {selectedBook.cover_image ? (
                        <img 
                          src={getOptimizedImageUrl(selectedBook.cover_image, { width: 400 })} 
                  ...
                  >
                    ...
                    <div className="md:hidden flex justify-center mb-4">
                      <div className="w-24 h-32 rounded-lg shadow-lg overflow-hidden">
                        {selectedBook.cover_image ? (
                          <img 
                            src={getOptimizedImageUrl(selectedBook.cover_image, { width: 200 })} 
                            alt={selectedBook.title}
                            className="w-full h-full object-cover"
                          />
                        ) : (
              ...
              >
                <div className="bg-gradient-to-br from-purple-900/95 to-[#1a1520]/95 backdrop-blur-lg rounded-2xl p-4 border border-purple-500/30 shadow-2xl shadow-purple-500/20 max-h-[60vh] overflow-y-auto">
                  ...
                  <div className="flex items-center justify-between mb-3 sticky top-0 bg-gradient-to-br from-purple-900 to-[#1a1520] -mx-4 -mt-4 px-4 pt-4 pb-2 rounded-t-2xl">
                    <div className="flex items-center gap-2">
                      <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-purple-400/50 bg-gradient-to-br from-purple-500 to-pink-500">
                        <img 
                          src="https://res.cloudinary.com/dlbmjqmoy/image/upload/e_background_removal/v1772279875/azories/mascot/azora_waving_hello.png"
                          alt="Azora"
